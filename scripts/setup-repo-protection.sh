...
#
# The protected branch is DERIVED FROM THE REPOSITORY's default branch at run
# time (never hardcoded), so renaming the default branch cannot silently leave
# the wrong branch unprotected.
#
...
# force-push the default branch, bypassing every rule above. The gate is
...
# BRANCH is derived from the repository's default branch unless the caller
...
# force-push the default branch, so the ruleset is absolute only for non-admin
